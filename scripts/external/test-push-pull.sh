#!/usr/bin/env bash
# Full push and pull smoke test — used with external S3 deployment (deploy-external.sh).
# Both push and pull work because Quay's blob redirect URLs use the external NooBaa
# hostname (s3-openshift-storage.apps.*) which is resolvable from outside the cluster.
...
echo "=== Push/Pull Smoke Test (external S3) ==="
...
# Remove local copy and pull back from Quay
echo "--- Pull from Quay ---"
podman rmi "${TARGET_TAG}" --force 2>/dev/null || true
if podman pull "${TARGET_TAG}" --tls-verify=false; then
  pass "Pulled ${TARGET_TAG} from Quay"
else
  fail "Pull from Quay failed"
  exit 1
fi
