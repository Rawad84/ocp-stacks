...
LOG_FILE="${REPO_ROOT}/deploy-full-scale-$(date +%Y%m%d-%H%M%S).log"
...
log "=== Starting Quay on OCP 4.20 deployment (full scale, internal S3) ==="
...
# Skip init job if admin token secret already exists and is non-empty
...
if [[ -n "${existing_token}" ]]; then
  ...
  # Delete any previous failed/completed job before re-creating
