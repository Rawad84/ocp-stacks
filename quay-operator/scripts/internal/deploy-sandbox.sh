#!/usr/bin/env bash
# Sandbox deployment — reduced resources, Clair disabled, HPA disabled.
# Uses internal NooBaa S3 hostname. Push works from outside; pull does not.
# Use deploy-external-sandbox.sh if you need pull from outside the cluster.
set -euo pipefail
...
LOG_FILE="${REPO_ROOT}/deploy-sandbox-internal-$(date +%Y%m%d-%H%M%S).log"
...
log "=== Starting Quay on OCP 4.20 deployment (sandbox mode) ==="
...
# ---------------------------------------------------------------------------
# Step 5 — Build Quay config bundle secret from OBC credentials
# ---------------------------------------------------------------------------

log "--- Step 5: Quay config bundle ---"

BUCKET_NAME=$(oc get configmap "${OBC_NAME}" -n "${QUAY_NAMESPACE}" \
              -o jsonpath='{.data.BUCKET_NAME}')
NOOBAA_S3_HOST=$(oc get configmap "${OBC_NAME}" -n "${QUAY_NAMESPACE}" \
                 -o jsonpath='{.data.BUCKET_HOST}')
NOOBAA_S3_PORT=$(oc get configmap "${OBC_NAME}" -n "${QUAY_NAMESPACE}" \
...
envsubst < "${REPO_ROOT}/manifests/02-quay-registry/quay-config-bundle-template.yaml" \
  | oc apply -f -
...
# ---------------------------------------------------------------------------
# Step 7 — QuayRegistry (sandbox flavor)
# ---------------------------------------------------------------------------

log "--- Step 7: QuayRegistry (sandbox) ---"
...
if [[ "${qr_condition}" == "True" ]]; then
  ...
  oc apply -f "${REPO_ROOT}/manifests/02-quay-registry/quay-registry-sandbox.yaml"
