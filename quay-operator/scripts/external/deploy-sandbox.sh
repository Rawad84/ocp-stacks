#!/usr/bin/env bash
# External-access sandbox deployment — reduced resources, Clair disabled, HPA disabled.
# Uses the external NooBaa S3 route so push and pull both work from outside the cluster.
# The cluster router CA cert is injected into the config bundle to trust the external route.
set -euo pipefail
...
LOG_FILE="${REPO_ROOT}/deploy-sandbox-external-$(date +%Y%m%d-%H%M%S).log"
...
log "=== Starting Quay on OCP 4.20 deployment (external + sandbox mode) ==="
...
log ""
log "=== Deployment complete (external + sandbox mode) ==="
